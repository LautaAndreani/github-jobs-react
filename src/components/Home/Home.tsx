...
const Home = ({ jobs, cards, setCards }: any) => {
  ...
  const locationFilter = (value: string) => {
    if (value === "Todos") return setCards(jobs);
    setCards(jobs.filter((card: any) => card.location.includes(value)));
  };
    ...
    <Box width={{ base: "100%", md: "90%" }} margin="0 auto" p={10}>
      ...
      <MotionBoxes>
        <Grid templateColumns={{ md: "repeat(2,1fr)", lg: "repeat(3,1fr)" }}>
          <GridItem colStart={1}>
            <Filter jobs={jobs} filterType={filterType} locationFilter={locationFilter} />
          </GridItem>
